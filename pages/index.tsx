...
const Item = dynamic(() => import('../components/overview/Item'), { loading: () => <Skeleton /> });
...
class Overview extends Component<Props, State> {
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      ...
      document
        .getElementById('shops')
        .addEventListener('wheel', this.onScroll.bind(this), { passive: true });
    }, 250);
  }
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      ...
      (this.props.filterExterior !== prevProps.filterExterior &&
        this.props.filterExterior.length > 0) ||
  ...
  onScroll() {
    ...
    if (
      wrappedElement.scrollHeight - wrappedElement.scrollTop === wrappedElement.clientHeight &&
          ...
          <div className='px-6 py-5 max-sm:p-2 max-sm:py-4 flex items-start flex-col w-full'>
            <div className='flex max-sm:block items-center w-full mb-3 max-sm:mb-2 max-sm:pl-2'>
              ...
              <div className='flex items-center'>
                <SortBy />
                <button className='flex items-center justify-center bg-almost_black rounded-lg ml-5 w-9 h-9'>
                  <Image alt='image' src='/icons/help.svg' width={18} height={18} />
                </button>
              </div>
            </div>
            <div className='w-full grid grid-cols-8 max-sm:grid-cols-1 gap-1'>
              {items.length > 0 ? (
